...
- 创建模型之间关联
  - 点击 "编辑拓扑" 进入可视化关联管理模式。
  - 画布左侧栏列出了当前系统的所有模型分组和模型，模型分组后面数字显示为 0，表示该模型分组下的所有模型都已经在模型拓扑画布里；数字为 m，表示该模型分组下还有 m 个模型没有在模型拓扑画布里，点开可以查看具体模型

   ![](images/cmdb-49.png)

   - 将模型拖入画布中，鼠标移动到模型上方，在浮出的按钮中，点击 【创建关联】 按钮，此时从当前模型到鼠标之间会出现一条虚线，鼠标选中其他任意模型，单机即可完成连线

   ![](images/cmdb-58.png)

   - 在关联连线完成后弹出的对话框中，填写 "关联描述"，根据需要修改其他参数即可创建关联（关联类型的创建可查看 本章的 [新增关联类型]（5.1/配置平台/产品功能/ModelManagement.md#新增关联类型））

   ![](images/cmdb-50.png)
   ![](images/cmdb-51.png)

- 删除模型之间关联
  - 点击 编 "辑拓扑" 进入可视化关联管理模式。
  - 直接点击模型之间连线，打开关联详情。
  - 点击 "删除关联" 完成删除关联
...
**注：**  
  - 如果两个模型之间有多种关联，鼠标指向模型关系的连线，在浮出的菜单中，选择需要编辑的关联进入到对应的详情页
